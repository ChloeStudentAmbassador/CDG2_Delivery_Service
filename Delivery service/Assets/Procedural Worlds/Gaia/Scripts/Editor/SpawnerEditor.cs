...
namespace Gaia
{
    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
        ...
        public void Spawn(bool allTerrains)
        {
            ...
            if (!allTerrains && Gaia.TerrainHelper.GetActiveTerrainCount() < 1 && !m_spawner.m_settings.m_isWorldmapSpawner)
            {
                EditorUtility.DisplayDialog("OOPS!", "You must have at least one terrain visible in order to do a local spawn. Please place your spawner over a terrain or add a new terrain from the Gaia Manager Window.", "OK");
                return;
            }
            else
            {
