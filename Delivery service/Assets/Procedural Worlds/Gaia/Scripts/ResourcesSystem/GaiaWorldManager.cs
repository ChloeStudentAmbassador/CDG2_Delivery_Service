...
namespace Gaia
{
    ...
    public class GaiaWorldManager
    {
        ...
        public GaiaWorldManager(Terrain[] terrains, bool isWorldMap = false)
        {
            ...
            {
                Debug.LogWarning("There was an error while handling undo data for this operation: " + validWorld + "\r\n This might affect undo data being stored for this stamping operation.");
                return;
            }
        ...
        {
            if (m_heightMapTerrainArray == null)
            {
                return;
            }
        ...
        public void ExportNormalmapAsPng(string path)
        {
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    PWCommon5.Utils.WriteAllBytes(spath, pngBytes);
            ...
            for (float x = minX; x < maxX; x += 1f)
            {
                ...
                for (float z = minZ; z < maxZ; z += 1f)
                {
                    ...
                    if (InBoundsTU(checkPos))
                    {
                        ...
                        {
                            strength = PWCommon5.Utils.Math_Distance(x, z, positionTU.x, positionTU.z) / maskSizeTU;
                            if (strength <= 1f)
                            {
