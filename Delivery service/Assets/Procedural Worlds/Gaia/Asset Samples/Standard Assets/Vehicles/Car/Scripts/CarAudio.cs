...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAudio : MonoBehaviour
    {
        ...
        [Range(0f, 1f)]
        public float m_carVolume = 0.5f;                                            // Sets the master volume of the audio
        public bool m_useTireSkidAudio = false;                                     //Sets weather skid audio will be used
        [Range(0f, 1f)]
        public float m_tireVolume = 0.25f;                                          // Sets the master volume of the audio
        ...
        private void Start()
        {
            m_currentTireVolume = m_tireVolume;
            m_currentUseTireSkidAudio = m_useTireSkidAudio;
            UpdateVolume();
        }
        ...
        private void Update()
        {
            ...
            if (m_StartedSound)
            {
                ...
                else
                {
                    ...
                    m_LowAccel.volume = Mathf.Clamp(lowFade*accFade, 0f, m_carVolume);
                    m_LowDecel.volume = Mathf.Clamp(lowFade*decFade, 0f, m_carVolume);
                    m_HighAccel.volume = Mathf.Clamp(highFade*accFade, 0f, m_carVolume);
                    m_HighDecel.volume = Mathf.Clamp(highFade*decFade, 0f, m_carVolume);
            ...
            if (m_currentTireVolume != m_tireVolume || m_currentUseTireSkidAudio != m_useTireSkidAudio)
            {
                m_currentTireVolume = m_tireVolume;
                m_currentUseTireSkidAudio = m_useTireSkidAudio;
                UpdateVolume();
        ...
        {
            if (m_wheels == null || m_wheels.Length != 4)
            {
                m_wheels = gameObject.GetComponentsInChildren<WheelEffects>();
            }

            if (m_wheels.Length > 0)
            {
                foreach (WheelEffects wheel in m_wheels)
                {
                    wheel.SetVolume(m_tireVolume, m_useTireSkidAudio);
                }
            }
        }
    }
